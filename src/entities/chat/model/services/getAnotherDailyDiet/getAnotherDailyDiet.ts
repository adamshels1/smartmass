...
import {dietPrompt} from 'entities/chat/model/types/promt.ts';
...
export const getAnotherDailyDiet = async () => {
  const request = 'Замени на другое, количество калорий должно остаться тем же';
...
function validateDietResponse(response: any) {
  ...
  if (parseInt(response.dishCalories) !== 450) {
